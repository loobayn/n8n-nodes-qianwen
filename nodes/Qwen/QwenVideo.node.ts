...
export class QwenVideo implements INodeType {
	...
	methods = {
		listSearch: {
			async searchVideoModels(this: ILoadOptionsFunctions, filter?: string): Promise<INodeListSearchResult> {
				...
				try {
					...
					if (response.data && Array.isArray(response.data)) {
						let models = response.data
							.filter((model: any) => model.id && model.id.startsWith('video'))
							.map((model: any) => ({
								name: model.id,
								value: model.id,
							}));
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (operation === 'getTaskStatus') {
					...
					// 视频生成需要使用原生API端点
					let videoEndpoint = credentials.apiEndpoint as string;
					...
					returnData.push({
						json: {
							...
							taskStatus: response.output?.task_status || 'PENDING',
