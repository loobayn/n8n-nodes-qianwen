...
class QwenImage {
    constructor() {
        ...
        this.methods = {
            listSearch: {
                async searchImageModels(filter) {
                    ...
                    try {
                        ...
                        if (response.data && Array.isArray(response.data)) {
                            let models = response.data
                                .filter((model) => model.id && model.id.startsWith('wanx'))
                                .map((model) => ({
                                name: model.id,
                                value: model.id,
                            }));
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                const negativePrompt = this.getNodeParameter('negative_prompt', i, '');
                ...
                let imageEndpoint = credentials.apiEndpoint;
                ...
                // 提取图片URL
                const images = ((_a = response.output) === null || _a === void 0 ? void 0 : _a.results) || [];
                returnData.push({
                    json: {
                        images: images,
                        taskId: ((_b = response.output) === null || _b === void 0 ? void 0 : _b.task_id) || '',
