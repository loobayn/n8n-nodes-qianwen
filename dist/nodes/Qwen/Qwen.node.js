                    ...
                    try {
                        ...
                        if (response.data && Array.isArray(response.data)) {
                            let models = response.data
                                .filter((model) => model.id && model.id.startsWith('qwen'))
                                .map((model) => ({
                                name: model.id,
                                value: model.id,
                            }));
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (additionalOptions.stop) {
                    requestBody.stop = additionalOptions.stop.split(',').map((s) => s.trim());
                }
                ...
                // 获取联网搜索开关（从顶层字段）
                const enableSearch = this.getNodeParameter('enable_search', i, false);
                if (enableSearch === true) {
                    requestBody.enable_search = true;
                }
                // 获取深度思考开关（从顶层字段）
                // 对于qwen3.5-plus等默认开启的模型，必须明确传递false
                const enableDeepThinking = this.getNodeParameter('enable_deep_thinking', i, false);
